...
import { DarkenPaletteService } from "@/domain/Entity/ColorValue/implementations/services/ColorPalette/LightnessPalette.service";
...
export const useColorServices = (): ColorServicesHookResult => {
  ...
  const conversionService = useMemo(
    () => new ColorConversionService(registry),
    [registry]
  );

  // Mémoriser le formateur de couleur
  const colorFormatter = useMemo(
    () => new ColorFormatService(conversionService),
    [conversionService]
  );

  // Mémoriser les services d'analyse
  const lightnessService = useMemo(
    () => new LightnessService(colorFormatter),
    [colorFormatter]
  );
  const saturationService = useMemo(
    () => new SaturationService(colorFormatter),
    [colorFormatter]
  );

  // Mémoriser les services d'amélioration
  const enhanceSaturationService = useMemo(
    () => new EnhanceSaturationService(colorFormatter),
    [colorFormatter]
  );
  const enhanceLightnessService = useMemo(
    () => new EnhancedLightnessService(colorFormatter),
    [colorFormatter]
  );

  // Mémoriser le service de contraste
  const contrastService = useMemo(
    () => new ColorContrastService(colorFormatter, lightnessService),
    [colorFormatter, lightnessService]
  );
