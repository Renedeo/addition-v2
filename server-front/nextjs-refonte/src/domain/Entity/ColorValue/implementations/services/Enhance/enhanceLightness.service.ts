...
export class EnhancedLightnessService implements IEnhancedColorService {
    ...
    enhanceColor(color: IColor, amount: number): IColor {
        ...
        const nextLightness = lightness + amount;
        if(nextLightness < 0 || nextLightness > 100) {
            throw new Error("Lightness enhancement out of bounds (0-100): " + nextLightness);
        }
        preferredFormat.value.l = lightness + amount;
