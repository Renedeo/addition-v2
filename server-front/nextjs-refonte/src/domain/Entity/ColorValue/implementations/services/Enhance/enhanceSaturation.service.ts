...
export class EnhanceSaturationService implements IEnhancedColorService {
    ...
    enhanceColor(color: IColor, amount: number): IColor {
        ...
        const nextSaturation = saturation + amount;
        if(nextSaturation < 0 || nextSaturation > 100) {
            throw new Error("Saturation enhancement out of bounds (0-100): " + nextSaturation);
        }
        preferredFormat.value.s = saturation + amount;
