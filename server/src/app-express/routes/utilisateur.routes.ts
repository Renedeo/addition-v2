...
export const utilisateurRoutes = (utilisateurService: UtilisateurService): Router => {
  ...
  router.get('/etablissement/:etablissementId', asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      throw new Error('Erreur lors de la récupération des utilisateurs de l\'établissement');
    }
  }));
  ...
  router.post('/', auditLogger('USER_CREATE'), asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (error.message.includes('nom')) {
        throw new ValidationError('Ce nom d\'utilisateur existe déjà');
      }
      throw error;
    }
  }));
   ...
   */
  router.put('/:id', auditLogger('USER_UPDATE'), asyncHandler(async (req: Request, res: Response) => {
    const userId = parseInt(req.params.id);
   ...
   */
  router.delete('/:id', auditLogger('USER_DELETE'), asyncHandler(async (req: Request, res: Response) => {
    const userId = parseInt(req.params.id);
